...
import 'package:relaisivoire/components/widgets/historique_bloc.dart';
...
import 'package:relaisivoire/pages/Tip.dart';
  ...
  void filterList(String query) {
    List<Colis> filtered = colisController.historique.value.where((colis) {
      ...
      return colis.code.toLowerCase().contains(lowerQuery) ||
          ...
          (colis.pointRelaisSender.target?.libelle.toLowerCase().contains(
                lowerQuery,
              ) ??
              false) ||
          (colis.pointRelaisSender.target?.adresse().toLowerCase().contains(
                lowerQuery,
              ) ??
              false) ||
          (colis.pointRelaisReceiver.target?.adresse().toLowerCase().contains(
                lowerQuery,
              ) ??
              false) ||
          (colis.pointRelaisReceiver.target?.libelle.toLowerCase().contains(
                lowerQuery,
              ) ??
              false);
    }).toList();
      ...
      body: SizedBox(
        child: Column(
          ...
          children: [
                ...
                child: displayedList.isNotEmpty
                    ? ListView(
                        children: displayedList
                            .map(
                              (colis) => HistoriqueBloc(colis: colis, tag: ""),
                            )
                            .toList(),
                      )
            ...
            Container(color: MyColors.secondary, height: Get.height / 10),
          ],
        ),
      ),
    );
